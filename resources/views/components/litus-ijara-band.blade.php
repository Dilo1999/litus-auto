...
<section {{ $attributes->merge(['class' => 'relative overflow-hidden bg-litus-paper-2 text-litus-text']) }}
    ...
    <div class="litus-container relative z-[2] litus-sec max-md:!py-12">
        {{-- Heading --}}
        <div class="litus-ijara-head mb-8 flex flex-wrap items-end justify-between gap-x-10 gap-y-5 sm:mb-10">
            <div class="max-w-[640px]">
                ...
                <p class="mt-3 text-[15px] leading-[1.6] text-litus-text-2 sm:text-[17px]">Choose your model, Ijara plan and number of months to see the down payment and monthly lease for that exact combination.</p>
            </div>
        ...
        <div class="grid items-start gap-5 min-[961px]:grid-cols-[1.5fr_1fr] min-[961px]:gap-7">
            {{-- Steps --}}
            <div class="rounded-[22px] border border-litus-line bg-white p-5 shadow-[0_1px_2px_rgba(9,17,32,.05),0_18px_44px_rgba(9,17,32,.07)] sm:p-8">
                ...
                <div data-step="model" class="grid gap-5 border-b border-litus-line pb-7 md:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] md:items-center">
                    <div>
                        ...
                        <p class="mt-2.5 text-[12.5px] text-litus-text-3" data-ijara-model-hint>Pick a motorcycle to see the plans offered for it.</p>
                    </div>
                ...
                <div data-step="plan" class="border-b border-litus-line py-7">
                    ...
                    <template data-ijara-plan-template>
                        <button type="button" aria-pressed="false" class="{{ $choice }} min-h-[84px]">
                ...
                {{-- 3. Term --}}
                <div data-step="term" class="pt-7">
                    <div class="mb-4 flex items-center gap-3">
                        <span class="{{ $stepBadge }}"><span data-num>3</span>{!! $tick !!}</span>
                        <span class="text-[16px] font-bold text-litus-text" id="ijara-term-label">Number of months</span>
                    </div>
                    <div class="grid grid-cols-2 gap-2.5 min-[560px]:grid-cols-3 lg:grid-cols-5" role="group" aria-labelledby="ijara-term-label" data-ijara-term>
                        @foreach ($terms as $term)
                            <button type="button" data-term="{{ $term }}" aria-pressed="false" class="{{ $choice }} min-h-[92px]">
                                <span class="flex items-start justify-between gap-1.5">
                                ...
                                <span class="mt-2 block whitespace-nowrap text-[11.5px] font-bold leading-snug text-litus-primary" data-term-monthly></span>
                                <span class="block text-[11px] leading-snug text-litus-text-3" data-term-down></span>
                            </button>
                        @endforeach
                    </div>
                    <p class="mt-3 text-[12.5px] text-litus-text-3">Down payment and monthly lease change with the bike, the plan and the number of months.</p>
                </div>
            </div>
            ...
            <aside class="overflow-hidden rounded-[22px] border border-litus-line bg-white shadow-[0_1px_2px_rgba(9,17,32,.05),0_18px_44px_rgba(9,17,32,.09)] min-[961px]:sticky min-[961px]:top-[96px]">
                <div class="relative overflow-hidden px-6 pb-6 pt-5 text-white"
                    ...
                    <p class="mt-3 text-[13px] text-white/70">Monthly lease</p>
                    ...
                    <p class="mt-2.5 text-[13px] leading-snug text-white/70" data-ijara-headline>Choose a model, plan and number of months.</p>
                </div>

                <div class="px-6 pb-6 pt-5">
                    <div class="rounded-xl border border-litus-line bg-litus-paper-2 px-4 py-3.5">
                        <span class="text-[12.5px] font-semibold text-litus-text-2">Down payment</span>
                        <b class="mt-0.5 block font-display text-[26px] font-extrabold leading-tight tracking-[-0.025em] text-litus-text" data-ijara-down>MVR -</b>
                    </div>

                    <dl class="mt-3 divide-y divide-litus-line text-[13.5px]">
                        ...
                        <div class="flex justify-between gap-4 py-3"><dt class="text-litus-text-2">Number of months</dt><dd class="text-right font-semibold text-litus-text" data-ijara-summary-term>-</dd></div>
                    </dl>

                    <a href="#" target="_blank" rel="noopener noreferrer" data-ijara-continue aria-disabled="true"
                       class="mt-3 flex min-h-[52px] w-full items-center justify-center gap-2 rounded-xl bg-litus-primary px-6 py-3 text-[15px] font-bold text-white shadow-[0_10px_24px_rgba(18,87,214,.32)] transition hover:-translate-y-0.5 hover:bg-litus-primary-hover aria-disabled:pointer-events-none aria-disabled:opacity-40 aria-disabled:shadow-none">
