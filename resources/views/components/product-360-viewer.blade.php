...
<div {{ $attributes->class(['group/spin relative h-full w-full touch-none select-none']) }}
    ...
    <div data-product-spin-hint
         @class([
             'pointer-events-none absolute bottom-3 left-1/2 z-[2] flex -translate-x-1/2 items-center gap-2 rounded-full border border-white/20 bg-black/45 px-3.5 py-1.5 text-[11px] font-bold uppercase tracking-wide text-white/90 opacity-0 backdrop-blur-sm transition-opacity duration-300 group-hover/spin:opacity-100 max-[1099px]:opacity-100 sm:text-xs',
             'hidden' => count($frames) <= 1,
         ])>
